...
def get_model_location(run_id, mlflow_server=False):
    ...
    if mlflow_server:
        uri = os.getenv('uri')
        name_registry = os.getenv('name_registry')
        name_experiment = os.getenv('name_registry')

        mlflow.set_tracking_uri(uri)
        client = mlflow.tracking.MlflowClient(tracking_uri=uri)
        model_location = client.get_latest_versions(name=name_registry, stages=['production'])[0]
        return model_location.source
